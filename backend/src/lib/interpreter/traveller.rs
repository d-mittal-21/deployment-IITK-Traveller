...
impl TravelStat {
    ...
    fn perform_operation(
        ...
        input: &mut Cursor<&[u8]>, output: &mut String
    ) -> Result<(), String> {
        ...
        match operation {
            2 => {
                ...
                match (*input).read_line(&mut buf){
                    Ok(_) => {}
                    Err(_) => {
                        return Err(format!("Error in reading input!"));
                    }
                }
                ...
                let i:i32 = match buf.trim().parse(){
                    Ok(i) => i,
                    Err(_) => {
                        return Err(format!("Error in parsing input"));
                    }
                };
            ...
            43 => {
                ...
                match (*input).read_line(&mut inp) {
                    Ok(_) => {},
                    Err(_) => {
                        return Err(format!("Failed to read input"));
                    }
                }
            ...
            56 => {
                ...
                let c:char = match buf.trim().parse(){
                    Ok(i) => i,
                    Err(_) => {
                        return Err(format!("Error in parsing input as char"));
                    }
                };
